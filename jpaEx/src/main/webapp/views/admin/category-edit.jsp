...
<div class="container">
  ...
  <form action="<c:url value='/${rolePath}/category/update'/>" method="post" enctype="multipart/form-data" class="card p-4">
    ...
    <div class="mb-3">
      ...
      <c:if test="${not empty cate.images}">
        <img src="<c:url value='/image?fname=${cate.images}'/>" width="120" class="img-thumbnail mb-2"/>
      </c:if>
    ...
    <div class="mb-3">
      ...
      <input type="radio" name="status" value="0" <c:if test="${cate.status == 0}">checked</c:if>> Khóa
    </div>
